...
public class LuaScriptEditor : MonoBehaviour
{
    ...
    internal static void CreateLua()
    {
        ...
        null,
@"
#NAME#=Object:Extend({TableName = '#NAME#'})
"
       );
    }
...
-- 对象池对象
#NAME# = ObjectItem:Extend({ TableName = '#NAME#'})
#NAME#.pool = ObjectPool.New(#NAME#)
...
-- Mono对象池对象
#NAME# = MonoObjectItem:Extend({TableName = '#NAME#'})
#NAME#.pool = MonoObjectPool.New(#NAME#,nil)

-- 获取对象，并设置父物体
function #NAME#.Get(parent)
    return #NAME#.pool:Get(parent).table
end

-- 获取对象
function #NAME#.Get()
    return #NAME#.pool:GetObject().table
end
